...
class Dashboard_Management extends MY_Controller {
		...
		if($type=="SOH"){
			...
			foreach ($result_drugs as $value) {
				...
				*/
				...
				$objPHPExcel -> getActiveSheet() -> SetCellValue('D'.$x, $cur_cons_kemsa);
				$objPHPExcel -> getActiveSheet() -> SetCellValue('E'.$x, $cur_cons_kp);
				$objPHPExcel -> getActiveSheet() -> SetCellValue('F'.$x, $avg_cons);
				//KEMSA
	...
	public function getPatients($type="ART_PATIENT",$period = ""){
		...
		elseif($type=="ADULT_ART"){//Bar Chart
			...
			if($period!=''){
				$period = date('Y-m-01',strtotime($period));
				$and = "WHERE m.period_begin <='$period' ";
			}
			$sql= "SELECT  
					IF(r.code='AF3A' OR r.code='AF3B' OR r.code='AF1A' OR r.code='AF1B' OR r.code='AF2A' OR r.code='AF2B',r.description,
						IF(r.code LIKE '%AS%','2nd line','Other Regimens'
						)
					) as regimen_cat,SUM(mi.total) as tot FROM sync_regimen r
					LEFT JOIN maps_item mi ON mi.regimen_id =r.id
					LEFT JOIN maps m ON m.id = mi.maps_id
					$and
		...
		elseif($type=="PAED_ART"){//Bar Chart
			...
			$sql= "
					...
					SELECT  
					IF(r.code='CF1A' OR r.code='CF1B' OR r.code='CF2A' OR r.code='CF2B' OR r.code='CF2D' OR r.code='CF1C',r.description,
						IF(r.code LIKE '%CS%','2nd line','Other Regimens'
						)
					) as regimen_cat,SUM(mi.total) as tot FROM sync_regimen r
					LEFT JOIN maps_item mi ON mi.regimen_id =r.id
					$and
