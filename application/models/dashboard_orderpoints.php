...
class Dashboard_Orderpoints extends Doctrine_Record {
	...
	public function getMonthlySummary($pipeline, $month, $year) {
		$query = Doctrine_Query::create() -> select("province,count(*) as total") -> from("dashboard_orderpoints") -> where("pipeline='$pipeline' and month='$month' and year='$year'") -> groupBy("province");
		$types = $query -> execute(array(), Doctrine::HYDRATE_ARRAY);
		return $types;
	}

	public function getMonthlyProvinceSummary($pipeline, $month, $year, $province, $summary) {
		$query = Doctrine_Query::create() -> select("count(*) as total") -> from("dashboard_orderpoints") -> where("pipeline='$pipeline' and month='$month' and year='$year' and province='$province' and $summary");
		$types = $query -> execute(array(), Doctrine::HYDRATE_ARRAY);
		return $types[0];
	}
